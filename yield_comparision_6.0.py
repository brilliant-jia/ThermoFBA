...
# 函数：绘制不同碳源在生长率为0.1附近时的yield与drG/catom的关系图
def plot_yield_vs_drg_per_atom():
    """
    绘制不同碳源在生长率为0.1附近时的yield与drG/catom的关系图
    """
    # 定义碳源颜色和标记
    carbon_sources = ['Glucose (Cathermo)', 'Glucose (Thermo)', 'Fructose', 'Pyruvate', 'Gluconate',
                      'Galactose']
    colors = ['red', 'darkgreen', 'blue', 'orange', 'purple', 'magenta']
    markers = ['*', '^', 'o', 's', 'd', 'p']
    ...
    growth_rate_tolerance = 0.02  # 0.08-0.12范围内的数据点
    ...
    for i, carbon_source in enumerate(carbon_sources):
        if carbon_source in simulation_data:
            ...
            # 查找目标生长率附近的数据点，并选择最接近0.1的一个
            closest_point = None
            min_distance = float('inf')

            for growth, yield_val, drG in zip(growths, yields, oxidation_drGs):
                if (abs(growth - growth_rate_target) <= growth_rate_tolerance and
                        drG is not None and yield_val is not None):
                    distance = abs(growth - growth_rate_target)
                    if distance < min_distance:
                        min_distance = distance
                        # 计算drG/catom
                        if carbon_source in c_dict:
                            carbon_atoms = c_dict[carbon_source]
                            drg_per_atom = drG / carbon_atoms
                            closest_point = (carbon_source, drg_per_atom, yield_val, colors[i], markers[i], growth, drG)

            # 如果找到符合条件的点，添加到数据点列表
            if closest_point:
                carbon_source, drg_per_atom, yield_val, color, marker, growth, drG = closest_point
                data_points.append((carbon_source, drg_per_atom, yield_val, color, marker))
                print(
                    f"{carbon_source}: 生长率={growth:.3f}, yield={yield_val:.3f}, drG={drG:.3f}, drG/catom={drg_per_atom:.3f}")

    # 绘制数据点
    for point in data_points:
        carbon_source, drg_per_atom, yield_val, color, marker = point
        plt.scatter(drg_per_atom, yield_val, color=color, marker=marker, s=150, label=carbon_source)

    # 添加标题和标签
    plt.title('Yield vs drG/catom at Growth Rate ~ 0.1', fontweight='bold', fontsize=18)
    plt.xlabel('drG/catom (kJ/mol C)', fontweight='bold', fontsize=16)
    plt.ylabel('yield (g$_{DW}$/g$_{substrate}$)', fontweight='bold', fontsize=16)
    ...
    # 保存图片
    plt.savefig(os.path.join(output_dir, 'yield_vs_drg_per_atom_0.1.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print("图片已保存: yield_vs_drg_per_atom_0.1.png")


def plot_oxidation_drg_vs_growth_rate():
    """
    绘制oxidation_drG随生长率的变化图
    """
    # 加载模拟数据
    simulation_data = load_or_process_simulation_data()
    ...
    # 定义颜色和标记
    colors = {
        'Glucose (Cathermo)': 'blue',
        'Glucose (Thermo)': 'green',
        'Fructose': 'red',
        'Pyruvate': 'purple',
        'Gluconate': 'orange',
        'Galactose': 'brown'
    ...
    # 绘制每个碳源的数据
    for carbon_source, data in simulation_data.items():
        growths = data['growths']
        oxidation_drGs = data['oxidation_drGs']

        # 过滤掉None值
        valid_data = [(g, drg) for g, drg in zip(growths, oxidation_drGs) if drg is not None]
        if valid_data:
            g, drg = zip(*valid_data)
            plt.scatter(g, drg, color=colors[carbon_source], marker=markers[carbon_source], s=100, label=carbon_source)
            plt.plot(g, drg, color=colors[carbon_source], linewidth=2)

    # 添加标题和标签
    plt.title('oxidation_drG vs Growth Rate', fontweight='bold', fontsize=18)
    plt.xlabel('Growth Rate (1/h)', fontweight='bold', fontsize=16)
    plt.ylabel('oxidation_drG (kJ/mol)', fontweight='bold', fontsize=16)
    ...
    # 保存图片
    plt.savefig(os.path.join(output_dir, 'oxidation_drg_vs_growth_rate.png'), dpi=300, bbox_inches='tight')


def plot_fermentation_drg_vs_growth_rate():
    """
    绘制fermentation_drG随生长率的变化图
    """
    # 加载模拟数据
    simulation_data = load_or_process_simulation_data()
    ...
    for carbon_source, data in simulation_data.items():
        growths = data['growths']
        fermentation_drGs = data['fermentation_drGs']

        # 过滤掉None值
        valid_data = [(g, drg) for g, drg in zip(growths, fermentation_drGs) if drg is not None]
        if valid_data:
            g, drg = zip(*valid_data)
            plt.scatter(g, drg, color=colors[carbon_source], marker=markers[carbon_source], s=100, label=carbon_source)
            plt.plot(g, drg, color=colors[carbon_source], linewidth=2)
    ...
    # 添加标题和标签
    plt.title('fermentation_drG vs Growth Rate', fontweight='bold', fontsize=18)
    plt.xlabel('Growth Rate (1/h)', fontweight='bold', fontsize=16)
    plt.ylabel('fermentation_drG (kJ/mol)', fontweight='bold', fontsize=16)
    ...
    # 保存图片
    plt.savefig(os.path.join(output_dir, 'fermentation_drg_vs_growth_rate.png'), dpi=300, bbox_inches='tight')
